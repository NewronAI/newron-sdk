...
git clone --branch ${SRC_BRANCH} --single-branch --depth 1 https://token:${PERSONAL_TOKEN}@github.com/${SRC_REPO}.git
if [ "$?" -ne 0 ]; then
...
git clone --branch ${DST_BRANCH} --single-branch --depth 1 https://token:${PERSONAL_TOKEN}@github.com/${DST_REPO}.git ${DST_REPO_DIR}
if [ "$?" -ne 0 ]; then
    echo >&2 "Cloning branch '$DST_BRANCH' in '$DST_REPO' failed"
    echo >&2 "Falling back to default branch"
    git clone --single-branch --depth 1 https://token:${PERSONAL_TOKEN}@github.com/${DST_REPO}.git ${DST_REPO_DIR}
